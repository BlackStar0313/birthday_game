...
@property (nonatomic, strong) NSTimer *frameCaptureTimer;
...
//    static dispatch_once_t once;
    static RSScreenVideo *sharedInstance;
    if (!sharedInstance) {
...
- (void) registerFileName:(NSString *)fileName
{
    self.fileName = fileName;
}
...
- (void)_startRecording {
    ...
    NSString *videoFileName = [NSString stringWithFormat:@"%@.mp4", self.fileName];
...
//    self.assetWriterVideoInput.transform = CGAffineTransformMakeScale(1.0f, -1.0f);
    ...
    self.assetWriterVideoInput.transform = [self videoTransformForDeviceOrientation];
...
//            [self cleanup];
       ...
       /* if (self.fileName) {
            ...
            NSLog(@"%@",_assetWriter.outputURL.path);
            [library writeVideoAtPathToSavedPhotosAlbum:_assetWriter.outputURL completionBlock:^(NSURL *assetURL, NSError *error) {
                if (error) {
                    NSLog(@"Error copying video to camera roll:%@", [error localizedDescription]);
                } else {
//                    [self removeTempFilePath:_assetWriter.outputURL.path];
                    completion();
                }
            }];
        }
...
- (void)captureFrame {
    ...
    if ((pixel_buffer == NULL) || (status != kCVReturnSuccess)) {
        ...
        if (1) {
            ...
            // swap dimesion
            //<<<
            int w = self.screenSize.height;
            int h = self.screenSize.height;
            
            int myDataLength = w * h *4;
            
            GLubyte *buffer = (GLubyte *)malloc(myDataLength);
            
            glReadPixels(0, 0, self.screenSize.width, self.screenSize.height, GL_BGRA_EXT, GL_UNSIGNED_BYTE, buffer);

            for(int y =0; y < self.screenSize.height; y++) {
                
                for(int x =0; x < w *4; x++) {
                    
                    pixelBufferData[(h -1- y) * w *4+ x] = buffer[y *4* w + x];
                }
            }
...
//            videoTransform = CGAffineTransformIdentity;
            break;
        default:
            // do mirror with x-axis
            videoTransform = CGAffineTransformScale(CGAffineTransformIdentity, 1, -1);
//            videoTransform = CGAffineTransformMakeRotation(M_PI);
            break;
    }
