...
static int json_escape_str(struct printbuf *pb, char *str)
{
  ...
  do {
    ...
    case '\0':
      break;
/*
 * commented by wyh: needn't escape such chars, as we won't use json in <script></script>
 *
 *
    case '\b':
    case '\n':
...
struct json_object* json_object_new_int(long long i)
{
...
long long json_object_get_int(struct json_object *jso)
{
  long long cint;
